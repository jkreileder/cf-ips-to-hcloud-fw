...
- `pre-commit` — a `.pre-commit-config.yaml` file is present and configured to run utilities like `ruff`, `gitleaks`, and `actionlint`. New contributors should install hooks with `pre-commit install` or `prek install`.
...
- Commit style — commit messages should follow Conventional Commits (`<type>[optional scope]: <description>`), e.g., `fix(auth): resolve login issue`.
- Please also format PR titles using Conventional Commits (`<type>[optional scope]: <description>`), for example `chore(deps): bump ruff to v0.14.6`.
